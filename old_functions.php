...
function pImplode ( $delimiter, $array, $prefix ) {
    foreach( $array as $x ){
        $oag[] = "$prefix$x";
    }
    return implode($delimiter, $oag);
}
...
function mailBody( string $scope )
{
    switch ( $scope ) {
        ...
        case "Quotation":
            ...
            $html    .=       '<div style="text-align: center; padding-top: 2em;">';
            $html    .=       '<div style="margin-top: 2em;">';
            $html    .=           'Sua cotação foi aceita, aguarde contato do fornecedor.';
            $html    .=        '</div>';
            $html    .=        '</div>';
            $html    .=   ' </div>';
            $html    .=   '<div style="flex-column m-auto">';
...
function readableFields () {
    return [ "doc", "fullName", "email", "tel", "uf", "city", "cep", "address", "docId", "companyDoc", "birth", "stateSubscription", "companyName", "id", "personId", "parentId", "userName", "verified", "suspended", "terms", "potentialScore", "dayForPayment", "orders", "scope", "walletId", "cities", "services" ];
}

function hashGenerate ( $pdo, string $userName, $mail ) {
    if ( $userName !== NULL ) {
        $sql = "SELECT account_personId FROM oag_accounts WHERE account_userName = '$userName'";
        $prepared = $pdo->prepare($sql);
        $prepared->execute();
        $result = $prepared->fetch();
        $personId = $result["account_personId"];
        $sql = "SELECT person_email FROM oag_people WHERE person_id = {$personId}";
        $prepared = $pdo->prepare($sql);
        $prepared->execute();
        $resulted = $prepared->fetch();
        $mailDocs = array($resulted["person_email"] . "xesque");
        $hash = hashGen( $mailDocs );
        $userEmail = $resulted["person_email"];
        $link = BASENAME . VALIDATION_PATH . "/mail/$userName/verify?hash=$hash";
        ...
        $mail->Body    .=           'Você se cadastrou para ter uma conta no orçaagora!';
        $mail->Body    .=            'Agora é só clicar no link abaixo e aproveitar as vantagens.';
...
function sqlExec( $pdo, string $sql ){
    $prepared = $pdo->prepare($sql);
    try {
        $prepared->execute();
        $response = $prepared->fetchAll();
    } catch (\Throwable $th) {
        $response = "Error";
    }
    return $response;
}
function sqlCon( $pdo, string $sql ){
    $prepared = $pdo->prepare($sql);
    try {
        $prepared->execute();
        $response = 0;
...
function getEnvironmentFromDir () {
    if ( end( explode( "/", __DIR__ ) ) === MAIN_DOMAIN ) {
        return "PROD";
    }
    if ( strpos( end( explode( "/", __DIR__ ) ), "back." ) !== false ) {
        return "TEST";
    }
    return "DEV";
};
